use super::{grid_vertex::GridVertex, grid_instance::GridInstance};


pub struct GridEntityMesh{
  pub vertices: Vec<GridVertex>,
  pub mesh_instance: [GridInstance; 1],
}

impl GridEntityMesh {
  pub fn new(vertices: Vec<GridVertex>, origin: [f32;3]) -> Self {
    Self {
      vertices,
      mesh_instance: [GridInstance { origin, scale: 1. }]
    }
  }
  ...
  pub fn to_vertex_buffer<T: bytemuck::Pod>(&self)->&[T]{
    bytemuck::cast_slice::<GridVertex, T>(&self.vertices)
  }

  pub fn to_instance_buffer<T: bytemuck::Pod>(&self)->&[T]{
    bytemuck::cast_slice::<GridInstance, T>(&self.mesh_instance)
  }
